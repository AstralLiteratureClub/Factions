...
@Cloud
public class PrefixSubCommand extends FactionCloudCommand {
	public PrefixSubCommand(Factions plugin, PaperCommandManager<CommandSender> commandManager) {
		...
		Command.Builder<Player> prefixBuilder = root
				.literal("prefix")
				.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX, "/factions prefix"))
		...
		Command.Builder<Player> reset = prefixBuilder.literal("reset")
				.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_RESET, "/factions prefix reset"))
				.handler(context -> {
					rootHelp.queryCommands("factions prefix reset", context.sender());
				});
		commandPlayer(reset);
		Command.Builder<Player> set = prefixBuilder.literal("set")
				.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET, "/factions prefix set"))
				.handler(context -> {
		...
		Command.Builder<Player> publicPrefix = set
				.literal("public")
				.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PUBLIC,
				...
				.required(MemberParser.memberComponent(MemberParser.Mode.OWN)
						.name("member")
						.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PUBLIC_MEMBER, "/factions prefix set public <member>"))
				)
				 */
				.required(RoleParser.roleComponent()
						.name("role")
						.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PUBLIC_ROLE, "/factions prefix set public <role>"))
				)
				.required(StringParser.stringComponent(StringParser.StringMode.QUOTED)
						.name("prefix")
						.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PUBLIC_PREFIX, "/factions prefix set public <role> <prefix>"))
				)
				.handler(context -> {
					...
					if (plain.length() > config.getPrefix().getMaxLength()) {
						messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PUBLIC_TOO_LONG, placeholders);
						return;
					}
					if (plain.length() < config.getPrefix().getMinLength()) {
						messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PUBLIC_TOO_SHORT, placeholders);
						return;
					}
					if (!plain.matches(config.getPrefix().getRegexPattern())) {
						messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PUBLIC_ILLEGAL, placeholders);
						return;
					}
					Component prefix = Component.text(plain);
					faction.setPublicPrefix(role, prefix);
					messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PUBLIC_CHANGED, placeholders);
				});
		...
		Command.Builder<Player> privateRoleBuilder =
				set
						.literal("private")
						.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE, "/factions prefix set private"))
		...
		commandPlayer(
				privateRoleBuilder
						.literal("role")
						.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_ROLE, "/factions prefix set private role"))
						...
						.required(
								RoleParser.roleComponent()
										.name("role")
										.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_ROLE_ROLE, "/factions prefix set private role <role>")))
						.required(StringParser.stringComponent(StringParser.StringMode.QUOTED)
								.name("prefix")
								.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_PREFIX, "/factions prefix set public <role> <prefix>"))
						)
						.handler(context -> {
							Player sender = context.sender();
							Faction faction = plugin.getPlayerManager().convert(sender).getFaction();
							...
							messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PRIVATE_ROLE_CHANGED, placeholders);
						})
		);
		...
		commandPlayer(
				privateRoleBuilder
						.literal("member")
						.commandDescription(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_PLAYER, "/factions prefix set private role"))
						...
						.required(
								MemberParser.memberComponent(MemberParser.Mode.OWN)
										.name("member")
										.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_PLAYER_MEMBER, "/factions prefix set private role <role>")))
						.required(StringParser.stringComponent(StringParser.StringMode.QUOTED)
								.name("prefix")
								.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_SET_PRIVATE_PREFIX, "/factions prefix set public <role> <prefix>"))
						)
						.handler(context -> {
							...
							messenger.message(sender, TranslationKey.MESSAGE_PREFIX_SET_PRIVATE_MEMBER_CHANGED, placeholders);
						})
		);
		commandPlayer(
				reset
						...
						.commandDescription(loadDescription(
								TranslationKey.DESCRIPTION_PREFIX_RESET_PUBLIC,
								"/factions prefix reset public"))
						.permission(PermissionUtils.of(FPermission.EDIT_ROLE_PREFIX_PUBLIC.getName(), FPermission.EDIT_ROLE_PREFIX_PUBLIC))
						.required(
								RoleParser.roleComponent()
										.name("role")
										.description(loadDescription(TranslationKey.DESCRIPTION_PREFIX_RESET_PUBLIC_ROLE, "/factions prefix reset public <role>"))
						)
						.handler(context -> {
							Player sender = context.sender();
							FRole role = context.get("role");
		...
		Command.Builder<Player> privateReset = reset.literal("private")
				.commandDescription(loadDescription(
						TranslationKey.DESCRIPTION_PREFIX_RESET_PRIVATE,
		...
		commandPlayer(
				privateReset
						.literal("role")
						.commandDescription(
								loadDescription(TranslationKey.DESCRIPTION_PREFIX_RESET_PRIVATE_ROLE, "/factions prefix reset private role"))
						.permission(
						...
						.required(
								RoleParser.roleComponent()
										...
										.description(loadDescription(
												TranslationKey.DESCRIPTION_PREFIX_RESET_PRIVATE_ROLE_ROLE,
		...
		commandPlayer(
				privateReset
						...
						.commandDescription(
								loadDescription(TranslationKey.DESCRIPTION_PREFIX_RESET_PRIVATE_PLAYER, "/factions prefix reset private role"))
						.permission(
								PermissionUtils.of(FPermission.EDIT_ROLE_PREFIX_PRIVATE.getName(), FPermission.EDIT_ROLE_PREFIX_PRIVATE))
						.required(
								MemberParser.memberComponent(MemberParser.Mode.OWN)
										.name("member")
										.description(loadDescription(
												TranslationKey.DESCRIPTION_PREFIX_RESET_PRIVATE_PLAYER_MEMBER,
												"/factions prefix reset private member <member>"))
						)
						.handler(context -> {
							Player sender = context.sender();
